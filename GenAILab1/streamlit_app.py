...
import grader    # noqa: E402  criterion-based grading engine (see grader.py)
import notebook  # noqa: E402  Jupyter-style multi-cell panel (see notebook.py)
...
def grade_task(code: str, t: dict, idx: int) -> dict:
    """LEGACY regex rubric (labs without criterion coverage in grader.py).
...
def accumulated_learner_source(spec: dict, upto_id: int, mode: str) -> str:
    """The learner's OWN code for Task 1..upto_id, concatenated with the sandbox
    preamble — exactly what grader.build_namespace() executes so runtime criteria
    inspect the real objects the submitted code produces."""
    codes = []
    for t in spec["tasks"]:
        codes.append(st.session_state.get(editor_key(t["id"]), t["starter"]))
        if t["id"] == upto_id:
            break
    return grader.assemble_source(codes, mode)


def evaluate(code: str, t: dict, idx: int, spec: dict, mode: str, dev: bool) -> dict:
    """Grade ONE task's submitted code. Uses grader.py's criterion engine when the
    lab/task has criteria defined (RAG lab today); otherwise the legacy regex
    rubric. Always returns {rows: [(desc, passed, earned, points)], got, max, full}."""
    tid = t["id"]
    lab_id = spec.get("lab_id", "")
    if grader.has_criteria(lab_id, tid):
        src = accumulated_learner_source(spec, tid, mode)
        res = grader.grade_task(tid, code, lab_id=lab_id, accumulated_source=src,
                                mode=mode, dev=dev)
        rows = [(f"{c['name']} — {c['reason']}", c["passed"],
                 float(c["earned"]), float(c["points"])) for c in res["criteria"]]
        return {"rows": rows, "got": float(res["score"]),
                "max": float(res["max_score"]), "full": res}
    return grade_task(code, t, idx)
...
#  UI                                                                          #
# --------------------------------------------------------------------------- #
def _key_status_line(mode: str, envk: str) -> None:
    """One tiny real API call to verify the key works — cached per key value.
    An invalid key is the #1 cause of 'nothing runs': surface it here, now."""
    import hashlib
    kh = hashlib.sha1(os.environ.get(envk, "").encode()).hexdigest()[:8]
    ck = f"_keychk_{mode}_{kh}"
    if st.button("Re-check key", key=f"recheck_{mode}", help="Make one tiny test call"):
        st.session_state.pop(ck, None)
    if ck not in st.session_state:
        with st.spinner(f"Testing {envk}…"):
            st.session_state[ck] = sandbox.check_llm(mode)
    ok, detail = st.session_state[ck]
    if ok:
        st.caption(f"✅ {detail}")
    else:
        st.error(f"**`{envk}` was rejected** — Runs will fail with this error:\n\n"
                 f"`{detail}`\n\nFix the key (env var / **Settings → Secrets**), press "
                 f"**Re-check key**, or switch to **Mock mode**.")
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if mode == "mock":
            st.caption("Demo fallback. **Real**: Python exec, LangChain, LangGraph, "
                       "vector store. **Stand-in**: only the answer LLM (no tool "
                       "calling — the agent labs need OpenAI or Claude).")
        else:
            ...
            if importlib.util.find_spec(pkg) is None:
                ...
                _key_status_line(mode, envk)
        st.session_state["mode"] = mode

        st.divider()
        _dev_default = bool(st.session_state.get("dev")) or \
            str(st.query_params.get("dev", "")).lower() in ("1", "true", "yes")
        dev = st.checkbox("🔧 Developer mode — grading diagnostics", value=_dev_default,
                          help="Show every rubric criterion (passed / earned / reason) "
                               "and the exact code the grader executed.")
    ...
    if kind == "overview":
        ...
        notebook.render_notebook(spec, mode, bool(st.session_state.get("dev")))
    elif kind == "score":
        render_score(spec)
    else:
...
def nav_bar(spec: dict):
    ...
    options = (["📋 Overview"] if has_ov else []) \
        + [f"Task {t['id']} · {t['title']}" for t in tasks] \
        + ["📓 Notebook", "★ Score"]
    last = len(options) - 1
...
def score_rows(spec: dict) -> tuple[list[dict], float]:
    ...
    for idx, t in enumerate(spec["tasks"]):
        ...
        cur_code = st.session_state.get(editor_key(tid))
        stale = bool(checked) and st.session_state.get(f"graded_code_{tid}") != cur_code
        revealed = bool(st.session_state.get(f"revealed_{tid}", False))
        if sub:
            tgot, status, ts = sub["got"], "submitted", sub["ts"]
        elif checked and not stale:
            tgot, status, ts = sum(r[2] for r in checked), "checked (not submitted)", ""
        elif checked and stale:
            tgot, status, ts = 0.0, "edited after check — re-check & submit", ""
        else:
...
def render_score(spec: dict) -> None:
    ...
    st.caption("Weights: "
               + " · ".join(f"T{r['id']} {r['weight']}" for r in rows)
...
def render_task(spec: dict, t: dict, mode: str) -> None:
    ...
    with left:
        ...
        if r2c2.button("↺ Reset", key=f"rst_{tid}"):
            ...
            for k in (f"result_{tid}", f"result_full_{tid}", f"graded_code_{tid}"):
                st.session_state.pop(k, None)
            st.rerun()

        dev = bool(st.session_state.get("dev", False))
        if do_check or do_submit:
            with st.spinner("Grading — analysing your code + running it…"):
                ev = evaluate(code, t, idx, spec, mode, dev)
            st.session_state[f"result_{tid}"] = ev["rows"]
            st.session_state[f"result_full_{tid}"] = ev["full"]
            st.session_state[f"graded_code_{tid}"] = code
        if do_submit:
        ...
        full = st.session_state.get(f"result_full_{tid}")
        stale = bool(rows) and st.session_state.get(f"graded_code_{tid}") != code
        if stale:
            st.warning("⚠ You edited the code since the last Check — press **✓ Check** "
                       "again to re-grade. The score below is from the previous version.")
        if rows:
            got = sum(r[2] for r in rows)
            st.markdown(f"**Check: {got:.1f} / {weight} marks**  ·  "
                        f"{sum(1 for r in rows if r[1])} / {len(rows)} criteria met")
            for desc, ok, pts, pmax in rows:
                st.markdown(f"{'✅' if ok else '❌'} {desc}  —  **{pts:g} / {pmax:g}**")
            if full is not None:
                rt = full.get("runtime", {})
                if not rt.get("executed"):
                    st.caption("⚠ Your Task 1…N code did not run to completion — "
                               f"runtime checks could not verify objects. "
                               f"{('Last error: `' + rt['error'] + '`') if rt.get('error') else ''}")
                if dev:
                    with st.expander("🔧 Grading diagnostics (developer mode)", expanded=True):
                        st.markdown(f"`grade_task(task_id={full['task_id']!r}, "
                                    f"lab_id={full['lab_id']!r})`  →  "
                                    f"**score {full['score']} / {full['max_score']}**")
                        st.markdown(f"runtime: executed=`{rt.get('executed')}` "
                                    f"error=`{rt.get('error')}`")
                        st.table([
                            {"criterion": c["name"],
                             "passed": "yes" if c["passed"] else "NO",
                             "earned": c["earned"], "points": c["points"],
                             "reason": c["reason"]}
                            for c in full["criteria"]
                        ])
                        if full.get("_source"):
                            st.caption("Exact source the grader executed:")
                            st.code(full["_source"], language="python")
                        if full.get("_run_output"):
                            st.caption("stdout / stderr from that run:")
                            st.code(full["_run_output"] or "(no output)", language="text")
